...
def train_rgb_ir(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # print(len(dataset.labels))  # 打印labels的长度
    # print(dataset.labels)  # 打印labels的内容
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                # if opt.quad:
                #     loss *= 4.
                # if opt.cfg.split(".")[0][-6:] == 'Fusion':
                #     pred, fusion = model(imgs_rgb, imgs_ir)
                #     vi_Y, vi_Cb, vi_Cr = RGB2YCrCb(imgs_rgb)
                #     ir_Y, ir_Cb, ir_Cr = RGB2YCrCb(imgs_ir)
                #     fusion_loss, int_loss, grad_loss, corr_loss = Fusion_loss(vi_Y, ir_Y, fusion,
                #                                                           device=device)
                #     loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
                #     loss = loss + 0.05 * fusion_loss
                #     if RANK != -1:
                #         loss *= opt.world_size  # gradient averaged between devices in DDP mode
                #     if opt.quad:
                #         loss *= 4.
                # elif opt.cfg.split(".")[0][-3:] == 'Res' or opt.cfg is None:

                ##############add Res Loss####################
                pred, res_vis, res_ir = model(imgs_rgb, imgs_ir)
                vi_Y, vi_Cb, vi_Cr = RGB2YCrCb(imgs_rgb)
                ir_Y, ir_Cb, ir_Cr = RGB2YCrCb(imgs_ir)
                vi_re_loss = Re_loss(res_vis, vi_Y, mask=None, ir_flag=False)
                ir_re_loss  = Re_loss(res_ir, ir_Y, mask=None, ir_flag=True)
                loss, loss_items = compute_loss(pred, targets.to(device))
                learnable_param = nn.Parameter(torch.tensor(0.05))
                learnable_param.data.clamp_(0, 1)
                loss = loss + learnable_param * (vi_re_loss + ir_re_loss)
                if RANK != -1:
                    loss *= opt.world_size  # gradient averaged between devices in DDP mode
                if opt.quad:
                    loss *= 4.
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='/home/zzh/CALNet/models/transformer/yolov5l_OBB_GraphNN_dronevehicle_Res.yaml', help='model.yaml path')   # BottleneckCSP !!!!!!!!!!!q
